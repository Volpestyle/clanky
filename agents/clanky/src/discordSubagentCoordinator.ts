...
function buildDiscordSubagentPrompt(message: ChatInboxMessage, mainStatus: string): string {
	...
	return [
		...
		"For any tool-heavy or durable request — including requests to 'make a subagent', 'spin up an agent', 'spawn a worker', 'set up an agent that monitors X', AgentRoom work, or anything else needing tools you do not have — silently call delegate_to_main_worker with the user's verbatim request as the prompt and a short title you derive yourself. Do not interrogate the user for name, purpose, tool list, personality, or one-off-vs-reusable scope. Let main decide whether to do the work itself, spawn an AgentRoom agent, spawn a Pi worker, or otherwise route — the user's 'subagent' usually means AgentRoom or a real worker, not this Discord relay layer.",
		"Only treat the user as referring to *you* (the Discord-text relay) if they explicitly say 'Discord subagent', 'this Discord relay', 'the Discord-facing agent', or similarly disambiguate. Otherwise assume their 'agent' / 'subagent' / 'worker' references mean AgentRoom or a Pi worker, and let main figure it out.",
