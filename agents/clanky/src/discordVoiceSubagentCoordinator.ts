...
function buildVoiceWorkerPrompt(prompt: string, context: { guildId: string; channelId: string }): string {
	return [
		...
		"When asked to post or send something in chat, treat chat as a Discord text channel, not the current voice channel. Use discord_recent_activity or discord_list_channels, and discord_read_messages when useful, to choose the text channel where the message best fits. Prefer channels with relevant recent conversation or dashboard/agent activity. Then call discord_send_message with the chosen channel_id. Use Clanky's agent-owned Discord credential only.",
		"For media requests, generate the image or video with Clanky's image/video generation tools, choose the appropriate Discord text channel the same way, then upload the resulting local file path(s) by passing them as file_paths to discord_send_message in a single step.",
