...
const TEXT_TOOL_SUMMARIES: Record<string, string> = {
  ...
  claim_task: "Claim an open swarm task as Clanky's controller peer.",
  ...
  unlock_file: "Release a file lock held by Clanky's controller peer.",
  ...
  whoami: "Return Clanky's current swarm controller peer identity.",
...
function buildCodeAgentDocs(): string[] {
  return [
    ...
    "If the user names a package or project fragment rather than a clear filesystem path, keep that in the task text and let the configured code-agent cwd anchor the worker. Relative cwd values resolve from that configured workspace, not from chat context.",
    ...
    "Clanky's own long-lived swarm peer is labeled role:controller. Treat it as the orchestrator identity for routing and cancellation, not as a coding worker to assign implementation/review/research tasks to.",
