...
test("Ollama fallback substitutes OpenAI model name with its own default", async () => {
  const store = createMockStore();
  let receivedModel = "";
  const openaiDown = createMockProvider({
    name: "openai",
    isReady: () => true,
  ...
  const ollama = createMockProvider({
    ...
    embed: async ({ model, input }) => {
      receivedModel = model;
      return { embedding: [1, 2, 3], model, inputTokens: input.length };
    }
  });

  await embedText(
    { store, providers: [openaiDown, ollama] },
    // Settings resolve to an OpenAI model name.
    { settings: { memory: { embeddingModel: "text-embedding-3-small" } }, text: "hello" }
  );

  // Ollama should have received its own default, not the OpenAI model name.
  expect(receivedModel).toBe("nomic-embed-text");
});

test("Ollama readiness recovers after recheck interval", async () => {
  // Directly test the provider's isReady behavior.
  const provider = createOllamaEmbeddingProvider("http://127.0.0.1:99999");
  // Initially not ready (no probe yet).
  expect(provider.isReady()).toBe(false);
  // After a failed embed, lastHealthCheckAtMs is set. Simulate by trying embed.
  try { await provider.embed({ model: "test", input: "test" }); } catch { /* expected */ }
  // Immediately after failure, still not ready (within cooldown).
  expect(provider.isReady()).toBe(false);
});
