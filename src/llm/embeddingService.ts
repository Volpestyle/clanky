...
const OLLAMA_HEALTH_RECHECK_INTERVAL_MS = 60_000;
...
export function createOllamaEmbeddingProvider(baseUrl?: string | null): EmbeddingProvider {
  ...
  return {
    ...
    isReady() {
      ...
      // Periodically allow a retry: if enough time has passed since the last
      // failed check, report ready so embed() gets a chance to re-probe.
      // This recovers automatically when Ollama comes up after startup.
      if (!lastHealthCheckAtMs) return false;
      return Date.now() - lastHealthCheckAtMs >= OLLAMA_HEALTH_RECHECK_INTERVAL_MS;
    },
    defaultModel() {
...
/**
 * Resolve the model to use for a specific provider. If the globally-resolved
 * model name looks like it belongs to a different provider (e.g. "text-embedding-3-small"
 * is an OpenAI model name), substitute the provider's own default so Ollama
 * doesn't try to load an OpenAI model.
 */
const OPENAI_EMBEDDING_MODEL_PATTERN = /^text-embedding-/i;

function resolveModelForProvider(globalModel: string, provider: EmbeddingProvider): string {
  if (provider.name === "openai") return globalModel;
  // If the global model looks like an OpenAI model name, use the provider's default.
  if (OPENAI_EMBEDDING_MODEL_PATTERN.test(globalModel)) return provider.defaultModel();
  return globalModel;
}
...
export async function embedText(
  ...
  const globalModel = resolveEmbeddingModel(deps, settings);
  let lastError: Error | null = null;

  for (const provider of chain) {
    if (!provider.isReady()) continue;
    const model = resolveModelForProvider(globalModel, provider);
    try {
  ...
  for (const provider of chain) {
    if (provider.isReady()) continue;
    const lastResortModel = resolveModelForProvider(globalModel, provider);
    try {
      const result = await provider.embed({ model: lastResortModel, input });
      const costUsd = estimateUsdCost({
        provider: provider.name,
        model: result.model,
        inputTokens: result.inputTokens,
