...
export async function browserSnapshot(
	...
	if (typeof input.tabId !== "number" || !Number.isFinite(input.tabId)) {
		throw new Error("browser_snapshot requires a finite tabId.");
	}
	const body: Record<string, unknown> = { tabId: input.tabId };
	for (const field of ["maxTextChars", "maxLinks", "maxMedia", "maxElements"] as const) {
		const value = input[field];
		if (value === undefined) continue;
		if (typeof value !== "number" || !Number.isFinite(value) || value < 0) {
			throw new Error(`browser_snapshot ${field} must be a non-negative finite number when provided.`);
		}
		body[field] = value;
	}
	const ctx = await prepareBridge(options);
	const record = await postBridge(ctx, "/snapshot", body, "snapshot");
	const tabId = record.tabId;
	const url = record.url;
	const title = record.title;
	const text = record.text;
	...
	if (
		...
		throw malformed("snapshot", JSON.stringify(record).slice(0, 200));
	}
	...
	if (
		...
		typeof counts.links !== "number" ||
		typeof counts.media !== "number" ||
		typeof counts.elements !== "number"
	) {
		throw malformed("snapshot", JSON.stringify(record).slice(0, 200));
	}
	return {
		...
		links: record.links as SnapshotLink[],
		media: record.media as SnapshotMedia[],
		elements: record.elements as SnapshotElement[],
		counts: {
			links: counts.links,
			media: counts.media,
