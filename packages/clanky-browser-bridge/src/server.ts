...
export async function startBrowserBridgeServer(options: BrowserBridgeServerOptions = {}): Promise<() => Promise<void>> {
	...
	async function handleHttp(req: IncomingMessage, res: ServerResponse): Promise<void> {
		...
		if (url.pathname === "/snapshot") {
			const params = requireRecord(parsed, "snapshot");
			const tabId = requireNumber(params, "tabId", "snapshot");
			const payload: Record<string, unknown> = { tabId };
			const maxTextChars = optionalNumber(params, "maxTextChars", "snapshot");
			if (maxTextChars !== undefined) payload.maxTextChars = clampSnapshotLimit(maxTextChars, 1, 100_000, "maxTextChars");
			const maxLinks = optionalNumber(params, "maxLinks", "snapshot");
			if (maxLinks !== undefined) payload.maxLinks = clampSnapshotLimit(maxLinks, 0, 500, "maxLinks");
			const maxMedia = optionalNumber(params, "maxMedia", "snapshot");
			if (maxMedia !== undefined) payload.maxMedia = clampSnapshotLimit(maxMedia, 0, 500, "maxMedia");
			const maxElements = optionalNumber(params, "maxElements", "snapshot");
			if (maxElements !== undefined) payload.maxElements = clampSnapshotLimit(maxElements, 0, 300, "maxElements");
			const result = await dispatch("snapshot", payload, SHORT_OP_TIMEOUT_MS);
			respondJson(res, 200, result);
			return;
		}
...
function clampSnapshotLimit(value: number, min: number, max: number, field: string): number {
	if (!Number.isFinite(value)) throw new Error(`snapshot field "${field}" must be finite.`);
