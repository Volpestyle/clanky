...
async function buildFixture(): Promise<string> {
	...
	for (const sub of ["runs", "events", "steps", "hooks", join("streams", "runs"), join("streams", "chunks")]) {
	...
	for (const runId of [STALE_DONE, STALE_ZOMBIE, FRESH]) {
		...
		await writeFile(join(dataDir, "hooks", `hook_${runId.slice(5)}.json`), JSON.stringify({ runId }));
...
try {
	...
	assert(
		remainingHooks.length === 2 && remainingHooks.includes("hook_01UNPARSABLE.json"),
