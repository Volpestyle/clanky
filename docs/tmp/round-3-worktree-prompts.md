...
- For dashboard route typing (Phase 3): define per-route-file dep interfaces. Use the shared Hono app type for the app param. The deps interface for each file should exactly match what that file destructures from deps. Check `src/dashboard/shared.ts` and `dashboard.ts` to see the full dep shape and define a `DashboardDeps` interface there too.
